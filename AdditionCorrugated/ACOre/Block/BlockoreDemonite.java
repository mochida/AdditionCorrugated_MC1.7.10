...
public class BlockoreDemonite extends Block implements IWorldGenerator
{
	
	public BlockoreDemonite()
	{
		super(Material.iron);
		setBlockName("OreDemonite");
		setBlockTextureName("additioncorrugated:block_oredemonite");
		setHardness(3.0F);
		setResistance(1.0F);
	...
	private void generateOre(World world, Random random, int x, int z)
	{
		for (int i = 0; i < 10; ++i)
		{
			...
			(new WorldGenMinable(AdditionCorrugated.ACOre.Block.Blocks.oreDemonite, 5)).generate(world, random, genX, genY, genZ);
		}
	}
}