...
public class BlockoreMythril extends Block implements IWorldGenerator
{
	
	public BlockoreMythril()
	{
		super(Material.iron);
		setBlockName("OreMythril");
		setBlockTextureName("additioncorrugated:block_oremythril");
		setHardness(2.0F);
		setResistance(1.0F);
	...
	private void generateOre(World world, Random random, int x, int z)
	{
		for (int i = 0; i < 10; ++i)
		{
			...
			(new WorldGenMinable(AdditionCorrugated.ACOre.Block.Blocks.oreMythril, 5)).generate(world, random, genX, genY, genZ);
		}
	}
}